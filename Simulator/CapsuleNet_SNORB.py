...
def load_snorb_bin():
    test_data_q_dir = str(pathlib.Path(__file__).parent.absolute()) + "/logs_snorb_tf/data.bin"
    test_labels_dir = str(pathlib.Path(__file__).parent.absolute()) + "/logs_snorb_tf/labels.bin"

    file_data = open(test_data_q_dir, "rb")
    data_buffer = file_data.read(24300*32*32*2)
    test_data = np.frombuffer(data_buffer, dtype='int8')
    test_data = test_data.reshape(-1,32,32,2).astype('float32')

    file_labels = open(test_labels_dir, "rb")
    labels_buffer = file_labels.read(24300)
...
def load_model_wt_bias(file_name):
    first_define = False
    wt_list = []
    dir = str(pathlib.Path(__file__).parent.absolute()) + "/logs_snorb_tf/" + file_name
...
def get_params():
    conv1_params = {'filters': 32, 'kernel_size': 7, 'stride': 1, 'activation': 'relu', 'out_shift': 9}

    pcaps_params = {'num_capsule': 16, 'dim_capsule': 4, 'kernel_size': 7, 'stride': 2,
                                                'squash_in_qn': 2, 'squash_out_qn': 7, 'out_shift': 10}

    caps_params = {'num_capsule': 5, 'dim_capsule': 6, 'num_routings': 3, 
                    'in_hat_shift': 7, 'out_ns_shifts': [10,11,11], 'b_inst_shifts': [8,8], 'b_new_shifts': [7,9],
                                                            'squash_in_qn': [6,5,5], 'squash_out_qn': [7,7,7]}
...
def main():
    test_data, test_labels = load_snorb_bin()
