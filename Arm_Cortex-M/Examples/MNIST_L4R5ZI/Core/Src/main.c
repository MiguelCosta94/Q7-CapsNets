...
int main(void)
{
  ...
  status_data_file = Open_File("mnist.bin", &USBHDataFile);
	if(status_data_file != FR_OK){
		return 0;
	}

  for(uint16_t i=0; i<DATASET_SIZE; i++) {
    status_data_file = Read_File_Batch("mnist.bin", &USBHDataFile, INPUT_DATA_SIZE, (char*)out_buffer2);
    if(status_data_file != FR_OK){
      break;
    }
