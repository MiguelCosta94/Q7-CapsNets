...
FRESULT Open_File (char *name, FIL *USBHFile2)
{
	/**** check whether the file exists or not ****/
	fresult = f_stat (name, &USBHfno);
	if (fresult != FR_OK)
	...
	{
		/* Open file to read */
		fresult = f_open(USBHFile2, name, FA_READ);
...
FRESULT Read_File_Batch (char *name, FIL *USBHFile2, int batch_size, char *buffer)
{
	/**** check whether the file exists or not ****/
	fresult = f_stat (name, &USBHfno);
	if (fresult != FR_OK)
	...
	else
	{
		/* Read data from the file
		* see the function details for the arguments */
		fresult = f_read (USBHFile2, buffer, batch_size, &br);
		if (fresult != FR_OK)
		{
			char *buf = malloc(100*sizeof(char));
			free(buffer);
