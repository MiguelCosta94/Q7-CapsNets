...
def get_q_format(min_val, max_val, optimization):
    ...
    if optimization:
        ...
        if max_val_abs!=0:
            ...
            #max_val_q7 = max_val_abs * pow(2, frac_bits)
...
def get_layer_io_range(layer, input_data):
    ...
    #for data_batch in data:
    #outlier_indexes = get_outlier_idx(input_data)
    #input_data = np.delete(input_data, outlier_indexes)
    #outlier_indexes = get_outlier_idx(output_batch)
    #output_batch = np.delete(output_batch, outlier_indexes)
...
def get_pcap_io_range(layer, input_data):
    ...
    #outlier_indexes = get_outlier_idx(input_data)
    #input_data = np.delete(input_data, outlier_indexes)
    #outliers_indexes = get_outlier_idx(output_batch)
    #output_batch = np.delete(output_batch, outliers_indexes)
    #output_batch_ns = np.delete(output_batch_ns, outliers_indexes)
...
def get_cap_io_range(layer, input_data):
    ...
    #input_outliers_idx = get_outlier_idx(input_data)
    #input_data = np.delete(input_data, input_outliers_idx)
    #input_hat_outliers_idx = get_outlier_idx(input_hat_batch)
    #input_hat_batch = np.delete(input_hat_batch, input_hat_outliers_idx)
    #for rout_idx in range(num_rout):
    #    output_ns_list_batch[rout_idx] = output_ns_list_batch[rout_idx].flatten()
    #    output_s_list_batch[rout_idx] = output_s_list_batch[rout_idx].flatten()
    #    output_ns_outliers_idx = get_outlier_idx(output_ns_list_batch[rout_idx])
    #    output_ns_list_batch[rout_idx] = np.delete(output_ns_list_batch[rout_idx], output_ns_outliers_idx)
    #    output_s_list_batch[rout_idx] = np.delete(output_s_list_batch[rout_idx], output_ns_outliers_idx)
...
def get_outlier_idx(batch):
    data_mean = np.mean(batch)
    data_std = np.std(batch)
    cut_off = data_std * 2

    lower = data_mean - cut_off
    upper = data_mean + cut_off

    outlier_idx_low = np.where(batch<lower)[0]
    outlier_idx_up = np.where(batch>upper)[0]
    outlier_idx = np.append(outlier_idx_low, outlier_idx_up)
