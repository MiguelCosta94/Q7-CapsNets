...
void mat_add_q7(matrix_instance_q7 *pSrcA, matrix_instance_q7 *pSrcB, uint16_t out_rshift, matrix_instance_q7 *pDst)
{
    ...
    while(eff_chunk>0)
    {
        /* Initiate pointer p_in_a to point to starting address of row being processed */
        p_in_a = pSrcA->pData + i;

        /* Initiate pointer p_in_b to point to starting address of row being processed */
