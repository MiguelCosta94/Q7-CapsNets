...
// pIm2ColBuffer size = [2*num_cores*ch_in*dim_kernel_x*dim_kernel_y]
void pulp_nn_conv_Co_parallel_int8(
  ...
  int8_t *pIm2ColBase = pIm2ColBuffer + (2 * core_id * ch_in * dim_kernel_x * dim_kernel_y);
  int8_t *pIm2Col = pIm2ColBase;
  int8_t *pW = pWeight + (start_channel * ch_in * dim_kernel_x * dim_kernel_y);

  int8_t *pOut = pOutBuffer + start_channel;
  ...
  if(eff_chunk)
  {
      ...
      for (i_out_x = 0; i_out_x < dim_out_x; i_out_x++)
      {
        if(i_out_y < padding_y_top)
        {
          ...
          for (i_ker_y = i_out_y * stride_y - padding_y_top; i_ker_y < i_out_y * stride_y - padding_y_top + dim_kernel_y; i_ker_y++)
          {
            for (i_ker_x = i_out_x * stride_x - padding_x_left; i_ker_x < i_out_x * stride_x - padding_x_left + dim_kernel_x; i_ker_x++)
            {
              ...
              {
                pulp_nn_im2col_int8((int8_t *) pInBuffer + (i_ker_y * dim_in_x + i_ker_x) * ch_in, pIm2Col, ch_in);
              }
              pIm2Col += ch_in;
            }
          }
        ...
        else if(i_out_y < dim_out_y - padding_y_bottom)
        {
          ...
          else if(i_out_x < dim_out_x - padding_x_right)
          {
            for (i_ker_y = i_out_y * stride_y - padding_y_top; i_ker_y < i_out_y * stride_y - padding_y_top + dim_kernel_y; i_ker_y++)
            {
              pulp_nn_im2col_int8((int8_t *) pInBuffer + (i_ker_y * dim_in_x + i_out_x * stride_x - padding_x_left) * ch_in, pIm2Col, ch_in * dim_kernel_x);
        ...
        {
          pOut = ((ch_out - eff_chunk) << 1) + pulp_nn_matmul_int8(
            pW,
            pIm2ColBase,
            eff_chunk,
    ...
    if (pIm2Col != pIm2ColBase)
    {
      ...
      for (i = start_channel; i < stop_channel; i++)
      {
        ...
        if (bias != NULL)
        {
          int bias_x = (int)(bias[i]);
          sum = (bias_x << bias_shift) + NN_ROUND(out_shift);
        }
        ...
        {
          *pOut = pulp_nn_bn_quant_int8(sum, *k0, *lambda0, out_shift);
          k0++;
          lambda0++;
          pOut++;
        }
