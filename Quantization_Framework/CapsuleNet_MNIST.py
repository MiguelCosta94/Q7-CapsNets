...
def caps_net(run_dir, hparams, train_data, train_labels, test_data, test_labels):
    with tf.summary.create_file_writer(run_dir).as_default():
        ...
        conv = layers.Conv2D(filters=hparams['conv_filters'], kernel_size=hparams['conv_kernel_size'],
                             strides=hparams['conv_stride'], padding='valid', kernel_initializer='he_uniform')(x)
        ...
        model.fit(x=train_data, y=train_labels, epochs=10, batch_size=32, verbose=1, workers=48,
...
def main():
    ...
    conv1_kernel_size = hp.HParam('conv_kernel_size', hp.Discrete([7]))
    ...
    tf.keras.models.save_model(model, 'caps_net_mnist.h5')
