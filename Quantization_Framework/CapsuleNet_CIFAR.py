...
def hp_tuning(hp_list, it_train, test_data, test_labels):
    ...
    for hp_set in hp_combinations:
        ...
        accuracy, model = caps_net(hp_log_dir + run_name, hp_set_to_test, it_train, test_data,
                                   test_labels)
...
def caps_net(run_dir, hparams, train_gen, test_data, test_labels):
    with tf.summary.create_file_writer(run_dir).as_default():
        ...
        conv1 = layers.Conv2D(filters=hparams['conv1_filters'], kernel_size=hparams['conv1_kernel_size'],
                             strides=hparams['conv1_stride'], padding='valid',
                             kernel_initializer='he_uniform', kernel_constraint=max_norm(0.5))(x)
        ...
        conv2 = layers.Conv2D(filters=hparams['conv2_filters'], kernel_size=hparams['conv2_kernel_size'],
                             strides=hparams['conv2_stride'], padding='valid',
                             kernel_initializer='he_uniform', kernel_constraint=max_norm(0.5))(conv1)
        ...
        conv3 = layers.Conv2D(filters=hparams['conv3_filters'], kernel_size=hparams['conv3_kernel_size'],
                             strides=hparams['conv3_stride'], padding='valid',
                             kernel_initializer='he_uniform', kernel_constraint=max_norm(0.5))(conv2)
        conv3 = layers.Activation(hparams['conv3_activation'])(conv3)

        conv4 = layers.Conv2D(filters=hparams['conv4_filters'], kernel_size=hparams['conv4_kernel_size'],
                             strides=hparams['conv4_stride'], padding='valid',
                             kernel_initializer='he_uniform', kernel_constraint=max_norm(0.5))(conv3)
        conv4 = layers.Activation(hparams['conv4_activation'])(conv4)
        ...
        pcaps = PrimaryCapsule(num_capsule=hparams['pcaps_num_capsule'], dim_capsule=hparams['pcaps_dim_capsule'],
                               ...
                               padding='valid', kernel_constraint=max_norm(0.5), name='primary_capsule')(conv4)
        ...
        model.fit(train_gen, epochs=100, batch_size=64, verbose=1, workers=48,
                   validation_data=(test_data[0:1000], test_labels[0:1000]), use_multiprocessing=True)
...
def main():
	...
	# prepare iterator
    train_gen = datagen.flow(train_data, train_labels, batch_size=64)
    ...
    conv2_stride = hp.HParam('conv2_stride', hp.Discrete([1]))
    ...
    model = hp_tuning(hp_list, train_gen, test_data, test_labels)

    tf.keras.models.save_model(model, 'caps_net_cifar.h5')
